...
from model.modules import PathSeqEncoder, SeqDecoder
...
class CFGSeq2Seq(LightningModule):
    ...
    def _get_encoder(self) -> PathSeqEncoder:
        return PathSeqEncoder(
    ...
    def forward(  # type: ignore
        ...
        batched_feature, batched_mask = self.encoder(
            samples.contexts,
        ...
        output = self.decoder(batched_feature, batched_mask, target_sequence.shape[0]-1, target_embedding)
        return output
    ...
    def training_step(self, batch: PathBatch, batch_idx: int) -> Dict:  # type: ignore
        ...
        statistic = PredictionStatistic(True, self._vocabulary.label_to_id[EOS], self._metric_skip_tokens)
    ...
    def validation_step(self, batch: PathBatch, batch_idx: int) -> Dict:  # type: ignore
        ...
        label_words = [self._vocabulary.id_to_label[index.item()] for index in labels[:,0]]
        prediction_words = [self._vocabulary.id_to_label[index.item()] for index in prediction[:,0]]
        print(label_words, prediction_words)
        statistic = PredictionStatistic(True, self._vocabulary.label_to_id[EOS], self._metric_skip_tokens)

...
class PredictionStatistic:
    ...
    def update_statistic_new(self, original: torch.Tensor, prediction: torch.Tensor) -> Dict[str, float]:
        ...
        for batch_idx in range(batch_size):
            ...
            gt_seq, pred_seq = sorted(gt_seq), sorted(pred_seq)
            common = Counter(gt_seq) & Counter(pred_seq)
            true_positive += sum(common.values())
            false_positive += (len(pred_seq) - true_positive)
            false_negative += (len(gt_seq) - true_positive)

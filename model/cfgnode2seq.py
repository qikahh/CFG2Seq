...
from model.modules import PathNodeEncoder, NodeDecoder
...
class CFGNode2Seq(LightningModule):
    ...
    def _get_encoder(self) -> PathNodeEncoder:
        return PathNodeEncoder(
    ...
    def _get_decoder(self) -> NodeDecoder:
        return NodeDecoder(
    ...
    def forward(  # type: ignore
        ...
        # path = [[token.item() for token in seq] for seq in samples.contexts.T]
        # contexts = [[self._vocabulary.id_to_token[token.item()] for token in seq] for seq in samples.contexts.T]
        # [max path length; totul paths, encoder_size],[max path length; batch size]
        expaths, expaths_mask = self.encoder(
            samples.contexts,
        ...
        output = self.decoder(expaths, expaths_mask, target_sequence.shape[0]-1, target_embedding)
        return output
